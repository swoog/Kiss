...
let rec findField members name = 
    match members with
    | [] -> raise(Exception("Not found"))
    | IlFieldBuilder(builder, n, _)::l -> if name = n then builder else (findField l name)
    | m::l -> (findField l name)
...
and buildEmitIl e ilGenerator locals classes members =
    ...
    | Stfld(name) -> ilGenerator.Emit(OpCodes.Stfld, ((findField members name)))
